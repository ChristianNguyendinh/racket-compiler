...
(define prims-list '(= > < <= >= + - * / 
                     ...
                     eq? eqv? equal? not
                     string string-append string-ref string->list substring))
...
(define err-rets (hash "tests/public/non-fun-0.err" "Error: Non-Function Value Applied"
                       "tests/public/non-fun-1.err" "Error: Non-Function Value Applied"
                       "tests/public/less-args-0.err" "was provided too few arguments"
                       "tests/public/less-args-1.err" "was provided too few arguments"
                       "tests/public/more-args-0.err" "as provided too many arguments"
                       "tests/public/more-args-1.err" "as provided too many arguments"
                       "tests/public/div-zero-0.err" "Error: Division by Zero"
                       "tests/public/div-zero-1.err" "Error: Division by Zero"
                       "tests/public/prim-bad-args-0.err" "Error: Invalid Number of Arguments Passed to a Primative"
                       "tests/public/prim-bad-args-1.err" "Error: Invalid Number of Arguments Passed to a Primative"))

(define (test-whole top-level
                    desugar
                    simplify-ir
                    assignment-convert
                    alphatize anf-convert
                    cps-convert
                    closure-convert
                    proc->llvm
                    top-level-e
                    path
                    ext)
  (define val1 (if (equal? ext "err")
                   (hash-ref err-rets path)
                   (eval-top-level top-level-e)))
  (define llvm (proc->llvm (closure-convert (cps-convert (anf-convert (alphatize (assignment-convert (simplify-ir (desugar (top-level top-level-e))))))))))
  (define val2 (eval-llvm llvm))
  ; this is actually really gross, but im lazy... dont look at it, close ur eyes
  (if (or (equal? val1 val2)
          (if (or (equal? path "tests/public/less-args-0.err")
                  (equal? path "tests/public/less-args-1.err")
                  (equal? path "tests/public/more-args-0.err")
                  (equal? path "tests/public/more-args-1.err"))
              (equal? val1 (substring val2 (- (string-length val2) 30) (string-length val2)))
              #f))
      #t
      (begin
        (display (format "Test-whole two different values (~a and ~a) before and after processing EVERYTHING :(\n"
                         val1 val2))
        #f)))
...
(define (simplify-ir ir-e)
  (define (T e)
    (match e
      ...
      [`(prim string)
       ''""]
      [`(prim string ,e0 ,e1 ,es ...)
       (T `(prim string-append ,@(map (lambda (x) `(prim string ,x)) (cons e0 (cons e1 es)))))]
