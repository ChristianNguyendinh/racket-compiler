...
(define (remove-varargs e vst) 
  (match e
         ...
         [`(let ([,x (lambda (,xs ...) ,body)]) ,e0)
          ...
          (define na (gensym 'na))
          (define ncx (gensym 'ncx))
          (define ssx (gensym 'ssx))
          (define tlerr (gensym 'tlerr))
          (define tserr (gensym 'tserr))
          (define gx+e
            (foldr (lambda (x gx+e)
                     (define gx (gensym 'rvp))
                     (define cx (gensym 'cx))
                           ...
                           `(let ([,cx (prim cons? ,gx)])
                                (if ,cx
                                    (let ([,x (prim car ,gx)])
                                      (let ([,(car gx+e) (prim cdr ,gx)])
                                        ,(cdr gx+e)))
                                    (let ([,sx (prim halt ,tserr)]) (,sx ,sx))))))
                   (cons na `(let ([,ncx (prim null? ,na)])
                               (if ,ncx
                                   ,(remove-varargs body vst)
                                   (let ([,ssx (prim halt ,tlerr)]) (,ssx ,ssx)))))
                   xs))
          `(let ([,tlerr ',(string-append "Error: Function " (symbol->string x) " was provided too many arguments")])
             (let ([,tserr ',(string-append "Error: Function " (symbol->string x) " was provided too few arguments")])
...
; hash of primative functions mapped to the number of arguments it MUST take
(define prim-params (hash "prim_print_aux" 1
                          "prim_print" 1
                          "prim_halt" 1
                          "prim_make_45vector" 2
                          "prim_vector_45ref" 2
                          "prim_vector_45set_33" 3
                          "prim_void" 0
                          "prim_eq_63" 2
                          "prim_eqv_63" 2
                          "prim_number_63" 1
                          ...
                          "prim_car" 1
                          "prim_cdr" 1
                          "prim_string_45append" 2
                          "prim_string" 1
                          "prim_string_45ref" 2
                          "prim_string_45_62list" 1
                          "prim_substring" 3
                          "prim__43" 2
                          "prim__45" 2
                          "prim__42" 2
      ...
      [`(let ([,x ',(? char? dat)]) ,e0)
           ...
           [`(let ([,x ',(? string? dat)]) ,e0)
            ...
            (string-append
             ...
             (e->llvm e0))]
           [`(let ([,x ',(? symbol? dat)]) ,e0)
            ;(when (eq? dat '%undefined) (raise (string-append (symbol->string x)
            ;                                                  " was not defined inside of letrec/letrec*. Cannot use before initialization" )))
           ...
           [`(let ([,x (prim ,op ,ys ...)]) ,e0)
            (define ll (length ys))
            (define pl (hash-ref prim-params (prim-name op)))
            (if (eq? ll pl) 
                (string-append
                 (comment-line
                 ...
                 (e->llvm e0))
                (string-append
                 (comment-line "  %" (s-> x) " = call i64 @prim_invalid()" "Invalid Primative Argument Error")
                 "  ret void"))]
           [`(let ([,x (apply-prim ,op ,y)]) ,e0)
            (string-append
             (comment-line
           ...
           [`(clo-app ,fx ,xs ...)
            ...
            (define pre (gensym 'pre))
            (define post (gensym 'post))
            (define cmp (gensym 'cmp))
            (define tlab (gensym 'then))
            (define flab (gensym 'else))
            
            (apply string-append
                   `(,(comment-line "  %" (s-> pre) " = call i64 @prim_procedure_63(i64 %" (s-> fx) ")" "Non-Function?")
                     ,(comment-line "  %" (s-> cmp) " = icmp eq i64 %" (s-> pre) ", 15" "false?")
                     ,(comment-line "  br i1 %" (s-> cmp) ", label %" (s-> flab) ", label %" (s-> tlab) "if")
                     ,(string-append "\n" (s-> tlab) ":\n")
                     ,(comment-line
                     ...
                     "  ret void\n"
                     ,(string-append "\n" (s-> flab) ":\n")
                     ,(comment-line "  %" (s-> post) " = call i64 @non_function()" "Non-Function?")
                     "  ret void"))]))
