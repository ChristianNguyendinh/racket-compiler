...
///// string, string->list, string-ref, substring, and string-append

u64 prim_string_45append(u64 a, u64 b)
{
    ASSERT_TAG(b, STR_TAG, "second argument to string-append must be a string")
    ASSERT_TAG(a, STR_TAG, "first argument to string-append must be a string")

    char* da = DECODE_STR(a);
    char* db = DECODE_STR(b);

    char* buffer = (char*)malloc(strlen(da) + strlen(db) + 1);

    strcpy(buffer, da);
    strcat(buffer, db);

    return ENCODE_STR(buffer);
}
...
u64 prim_string(u64 a)
{
    ASSERT_TAG(a, CHAR_TAG, "first argument to string must be a character")

    char* buffer = (char*)malloc(2);
    buffer[0] = DECODE_CHAR(a);
    buffer[1] = 0;

    return ENCODE_STR(buffer);
}

u64 applyprim_string(u64 p)
{
    if (p == V_NULL) 
    {
        char* t = (char*)malloc(1);
    ...
    {
        ASSERT_TAG(p, CONS_TAG, "Tried to apply string on non list value.")
        u64* pp = DECODE_CONS(p);
        return prim_string_45append(prim_string(pp[0]), applyprim_string(pp[1]));
    }
}

u64 prim_string_45ref(u64 s, u64 i)
{
    ASSERT_TAG(i, INT_TAG, "second argument to string-ref must be a integer")
    ASSERT_TAG(s, STR_TAG, "first argument to string-ref must be a string")

    return ENCODE_CHAR((s32)(DECODE_STR(s)[DECODE_INT(i)]));
}
GEN_EXPECT2ARGLIST(applyprim_string_45ref, prim_string_45ref)

u64 prim_string_45_62list(u64 s)
{
    ASSERT_TAG(s, STR_TAG, "first argument to string->list must be a string")

    char* ds = DECODE_STR(s);
    u64 c = const_init_null();

    for (int i = strlen(ds) - 1; i >= 0; i--) {
        c = prim_cons(ENCODE_CHAR(ds[i]), c);
    }
...
u64 prim_substring(u64 s, u64 b, u64 e)
{
    ASSERT_TAG(e, INT_TAG, "third argument to substring must be a integer")
    ASSERT_TAG(b, INT_TAG, "second argument to substring must be a integer")
    ASSERT_TAG(s, STR_TAG, "first argument to substring must be a string")
    ...
    if (db > de) {
        fatal_err("Third integer argument to substring cannot be less than the second argument");
    }
    else if (db < 0)
    {
        fatal_err("second integer argument to substring cannot be less than 0");
    }
    else if (de > strlen(ds))
    {
        fatal_err("Third integer argument to substring cannot be greater than the length of the string");
    }

    char* n = (char*)malloc((de - db) + 1);

    for (int i = 0; i < (de - db); i++) {
        n[i] = ds[i + db];
    }
...
GEN_EXPECT3ARGLIST(applyprim_substring, prim_substring)
...
u64 prim__47(u64 a, u64 b) // /
{
    ...
    if (DECODE_INT(b) == 0) {
        char* a = (char*)malloc(31);
        strcpy(a, "Error: Division by Zero");
        prim_halt(ENCODE_STR(a));
    }
    return ENCODE_INT(DECODE_INT(a) / DECODE_INT(b));
}
...
u64 prim_invalid()
{
    char* a = (char*)malloc(57);
    strcpy(a, "Error: Invalid Number of Arguments Passed to a Primative");
    prim_halt(ENCODE_STR(a));
...
u64 non_function()
{
    char* a = (char*)malloc(34);
    strcpy(a, "Error: Non-Function Value Applied");
    prim_halt(ENCODE_STR(a));
